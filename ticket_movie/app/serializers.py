...
class CinemaSerializer(serializers.ModelSerializer):
    ...
    def validate_phone(self, value):
        if not value.strip() or len(value) < 10:
            raise serializers.ValidationError(
                "Please enter cinema phone number with at least 10 digits.")
        return value
...
class ScreenSerializer(serializers.ModelSerializer):
    ...
    def validate_type(self, value):
        if len(value) > 20:
            raise serializers.ValidationError(
                "Please enter cinema type with at most 20")
        return value

    def validate_capacity(self, value):
        if value < 1:
            raise serializers.ValidationError(
                "Please enter cinema capacity number with at least 1 digits.")
        return value
...
class ShowtimeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        try:
            ...
            raise serializers.ValidationError(
                "Invalid date format. Use ISO format: YYYY-MM-DDTHH:MM:SS±HH:MM")
